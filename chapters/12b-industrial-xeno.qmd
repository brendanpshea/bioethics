...
![](../images/cafo_aerial.jpg){width="78%" fig-alt="Aerial view of a concentrated animal feeding operation: rows of long metal barns surrounded by bare ground and waste lagoons"}
...
# Sequential ramp: attrition is ordered, so one hue light -> dark, not three hues.
colors = ["#93d1d8", "#3fa8b4", "#0e7c86"]
texts  = ["#0f172a", "#0f172a", "#ffffff"]
for i, (s, w, c, tc) in enumerate(zip(stages, widths, colors, texts)):
    left = (100 - w) / 2
    ax.barh(2 - i, w, left=left, color=c, edgecolor="#0e7c86", linewidth=0.8)
...
digraph xeno {
  ...
  node [shape=box, style="rounded,filled", fontname="Inter", fontsize=11,
        fillcolor="#e6f1f2", color="#0e7c86", fontcolor="#0f172a",
        penwidth=1.4, margin="0.20,0.13"];
  edge [color="#0e7c86", arrowhead=vee, penwidth=1.3];
  a [label="Knock OUT pig genes\n• alpha-gal & other sugars\n  (trigger instant rejection)"];
  b [label="Inactivate PERVs\n• pig retroviruses in the\n  genome (infection risk)"];
  c [label="Knock IN human genes\n• regulate clotting &\n  inflammation"];
  d [label="Pig organ tolerated\nby the human body",
     fillcolor="#0e7c86", fontcolor="#ffffff", color="#095058"];
  a -> b -> c -> d;
}
```
...
```{python}
#| label: fig-xeno-survival
#| fig-align: center
#| fig-width: 9.5
#| fig-height: 4.0
#| echo: false
#| fig-alt: "Bar chart of days survived with a gene-edited pig organ. Bennett 2022 about 60 days; Faucette 2023 about 42; Slayman 2024 about 60; Looney 2024-25 130 days; Andrews 2025-26 271 days."
import matplotlib.pyplot as plt

# Update this list each term. Days the transplanted organ functioned.
rows = [
    ("David Bennett",     "2022 · heart",     60,  True),   # True = approximate
    ("Lawrence Faucette", "2023 · heart",     42,  True),
    ("Rick Slayman",      "2024 · kidney",    60,  True),
    ("Towana Looney",     "2024–25 · kidney", 130, False),
    ("Tim Andrews",       "2025–26 · kidney", 271, False),
]

TEAL, INK, MUTED, RULE = "#0a95a3", "#0f172a", "#64748b", "#e5e7eb"
plt.rcParams["font.family"] = ["Inter", "DejaVu Sans", "sans-serif"]

fig, ax = plt.subplots(figsize=(9.5, 4.0))
labels = [f"{name}\n{meta}" for name, meta, _, _ in rows]
days   = [d for _, _, d, _ in rows]
ypos   = range(len(rows))

ax.barh(ypos, days, height=0.62, color=TEAL, zorder=3)

for y, (_, _, d, approx) in zip(ypos, rows):
    ax.text(d + 6, y, f"{'~' if approx else ''}{d}", va="center",
            fontsize=11, fontweight="600", color=INK, zorder=4)
...
ax.set_xlabel("days the organ functioned", fontsize=10, color=MUTED, labelpad=8)

ax.xaxis.grid(True, color=RULE, linewidth=1, zorder=0)
ax.set_axisbelow(True)
for side in ("top", "right", "left"):
    ax.spines[side].set_visible(False)
ax.spines["bottom"].set_color(RULE)

ax.set_title("How long each pig organ lasted", fontsize=14, fontweight="700",
             color=INK, loc="left", pad=14)
fig.text(0.005, -0.03,
         "Andrews received a human kidney in January 2026 — the first time a pig organ "
         "bridged a patient to a human transplant.",
...
- **From rescue to trial.** In 2025 the FDA cleared the first real xenotransplant trials. Xeno now has a **clinical equipoise** problem it did not have before.
- **Andrews crossed a bridge.** 271 days on a pig kidney — then a *human* one.
:::
...
**2. Tim Andrews crossed a bridge.** Andrews, a 67-year-old retired engineer from New Hampshire, received a 69-gene-edited pig kidney in January 2025. He lived with it **271 days** — surpassing Looney's 130 — before declining function forced its removal in October 2025 and he returned to dialysis [@cnn2025_andrews_271days]. Then, in **January 2026**, he received a **human** kidney [@cnn2026_xeno_bridge].
...
digraph threads {
  ...
  node [shape=box, style="rounded,filled", fontname="Inter", fontsize=10,
        fillcolor="#e6f1f2", color="#0e7c86", fontcolor="#0f172a",
        penwidth=1.4, margin="0.18,0.11"];
  edge [color="#94a3b8", arrowhead=vee, penwidth=1.2];
  center [label="Does it apply\nto animals?", shape=ellipse,
          fillcolor="#0e7c86", fontcolor="#ffffff", color="#095058"];
  a [label="Abortion:\n'future like ours'\n(Marquis)"];
  g [label="Genetics:\nmaking a being\nto be used"];
  e [label="End of life:\nkilling for benefit"];
